...
from .app_utils import (
    predict,
    explain_gradient,
    explain_input_x_gradient,
    explain_integrated_gradients,
    explain_gifsplanation,
)
...
def main():
    ...
    frontend.launch(
        ...
        share=True,  # creates a (temporary) public link on https://gradio.app
    )
...
def make_frontend():
    ...
    example_fnames = [
        elem for elem in os.listdir(examples_dir) if elem.endswith(".png")
    ]
    ...
    with frontend:
        gr.Markdown(
            """
            # XRayMind: Prognostics for Chest Radiography via Transfer Learning
            Human-in-the-loop interface for exploring TorchXRayVision models and data.
            This automated tool allows users to diagnose chest radiographs by selecting their choice of model and explaining the model's predictions.
            Users can simply upload an image of a chest x-ray into the algorithm to create a report.
            In the example, there are 3 chest x-ray images available from the NIH dataset that can be used to test the algorithm.
            """
        )
        # layout
                ...
                with gr.Column():
                    with gr.Tab("Input"):
                        ...
                        select_model = gr.Dropdown(
                            label="Select model", choices=model_choices
                        )
                        with gr.Row():
            ...
            with gr.Row():
                with gr.Column():
                    select_target = gr.Dropdown(
                        label="Select target", choices=target_choices
                    )
                    with gr.Row(equal_height=True):
                        with gr.Column():
                            with gr.Tab("Im..."):
                                original_image = gr.Image(
                                    label="Original", interactive=False
                                )
                        with gr.Column():
                            with gr.Tab("Gra..."):
                            ...
                            with gr.Tab("Int..."):
                                integrated_gradients_plot = gr.Plot(
                                    label="IntegratedGradients"
                                )
                            with gr.Tab("Gif..."):
        ...
        select_target.change(
            explain_gradient,
            inputs=[input_image, select_model, select_target],
            outputs=gradient_plot,
        )
        select_target.change(
            explain_input_x_gradient,
            inputs=[input_image, select_model, select_target],
            outputs=input_x_gradient_plot,
        )
        select_target.change(
            explain_integrated_gradients,
            inputs=[input_image, select_model, select_target],
            outputs=integrated_gradients_plot,
        )
        select_target.change(
            explain_gifsplanation,
            inputs=[input_image, select_model, select_target],
            outputs=gifsplanation_vid,
        )
