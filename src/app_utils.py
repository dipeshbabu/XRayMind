...
model_ensemble = ChestXrayEnsemble(
    num_classes=n_classes, base_learners=base_learner_names
)
...
def predict_ensemble(image):
    ...
    result = [
        a for a in zip(model_ensemble.pathologies, scores, diagnosis) if a[0] != ""
    ]
    d = {
        "Pathology": [a[0] for a in result],
        "Score": [a[1] for a in result],
        "Diagnosis": [a[2] for a in result],
    }
...
def predict(image, model_choice):
    ...
    if model_choice == "torchxrayvision-ensemble":
        ...
        result = [
            a for a in zip(model.pathologies, scores, diagnosis) if a[0] != ""
        ]  # remove empty pathologies
        d = {
            "Pathology": [a[0] for a in result],
            "Score": [a[1] for a in result],
            "Diagnosis": [a[2] for a in result],
...
def explain_gifsplanation(image, model_choice, target):
    ...
    movie = attribution.generate_video(
        input,
        model,
        target,
        ae,
